...
import { AgendaEventTransition } from '../../../agenda/domain/services/eventStateMachine.service';
...
export class ConfirmationCheckerJob extends BaseComponent {
  ...
  private async sendConfirmationReminders() {
    ...
    for (const event of pendingEvents) {
      const hoursRemaining = this.calculateHoursUntilExpiration(event.createdAt);
    ...
    this.logger.log(`Processed ${pendingEvents.length} events for confirmation reminders`);
  }
  ...
  private async autoCancelExpiredEvents() {
    const expiredEvents = await this.agendaEventRepository.findExpiredPendingEvents();
    ...
    for (const event of expiredEvents) {
      try {
        ...
        await this.sendAutoCancellationNotification(event, 'NO_CONFIRMATION_48H');
  ...
  private async sendConfirmationReminder(event: any, hoursRemaining: number) {
    try {
      ...
      this.logger.debug(`Queued confirmation reminder for event ${event.id}, ${hoursRemaining}h remaining`);
    } catch (error) {
      this.logger.error(`Failed to queue confirmation reminder for event ${event.id}:`, error);
    }
  }
  ...
  private async sendAutoCancellationNotification(event: any, reason: string) {
    try {
      ...
      this.logger.debug(`Queued auto-cancellation notification for event ${event.id}`);
    } catch (error) {
      this.logger.error(`Failed to queue auto-cancellation notification for event ${event.id}:`, error);
    }
  }
  ...
  private calculateHoursUntilExpiration(createdAt: Date): number {
    ...
    return Math.max(0, (expirationTime.getTime() - now.getTime()) / (1000 * 60 * 60));
  }
} 