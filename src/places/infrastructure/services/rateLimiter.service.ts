...
export class RateLimiterService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.maxRequestsPerMinute = this.configService.get(
      'places.rateLimitPerMinute',
      30,
    );
    this.maxRequestsPerHour = this.configService.get(
      'places.rateLimitPerHour',
      1000,
    );
    this.cacheExpiry = this.configService.get('places.cacheExpiry', 3600); // 1 hour default
  }

  async tryAcquire(identifier = 'global'): Promise<boolean> {
    const now = new Date();
