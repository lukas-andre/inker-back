...
import { DomainBadRequest, DomainBadRule, DomainNotFound } from "../../../global/domain/exceptions/domain.exception";
...
export class SendForgotPasswordCodeUseCase implements UseCase {
    ...
    async execute(
        ...
        if (!email && !phoneNumber) {
            throw new DomainBadRequest('At least one identifier (email, phone number, or username) must be provided');
        }
        ...
        const notificationType = email ? NotificationType.EMAIL : NotificationType.SMS;
        ...
        if (existingHash) {
            if (existingHash.tries >= SendForgotPasswordCodeUseCase.THRESHOLD[notificationType]) {
        ...
        const noficationData: SendVerificationCodeJobType = {
            ...
            metadata: {
                ...
                expirationTime: SendForgotPasswordCodeUseCase.THRESHOLD[notificationType],
            }
        }
