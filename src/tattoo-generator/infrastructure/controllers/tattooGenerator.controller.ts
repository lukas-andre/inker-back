...
export class TattooGeneratorController {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves the current user\'s generated tattoo design history with optional filtering for favorites',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'favorites', required: false, type: Boolean, description: 'Filter to show only favorites' })
  ...
  @ApiOperation({
    ...
    description: 'Mark or unmark a tattoo design as favorite for the current user',
  })
  @ApiResponse({
    status: 200,
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async updateFavorite(@Body() updateFavoriteDto: UpdateTattooFavoriteDto): Promise<void> {
