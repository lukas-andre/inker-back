...
import {
  ...
  getColorInstruction,
} from '../../domain/enums/tattooStyle.enum';
import {
  IPromptEnhancementService,
  PromptEnhancementParams,
} from '../../domain/interfaces/prompt-enhancement.interface';

import { LibreTranslationService } from './libreTranslation.service';

@Injectable()
export class TattooPromptEnhancementService
  implements IPromptEnhancementService
{
  private readonly logger = new Logger(TattooPromptEnhancementService.name);

  constructor(private readonly translationService: LibreTranslationService) {}
  ...
  async enhancePrompt(params: PromptEnhancementParams): Promise<string> {
    ...
    try {
      const translatedInput = userInput
        ? await this.translationService.translate(userInput, 'auto', 'en')
        : 'a unique design';
      this.logger.log(`Translated user input: ${translatedInput}`);

      const styleDescriptionRaw =
        TattooStyleDescriptions[style as TattooStyle] || '';
      const styleKeywords = styleDescriptionRaw
        .split('|')
        .map(s => s.trim())
        .filter(s => s.length > 0);
      ...
      this.logger.log(
        `Color instruction for style ${style}: ${colorInstruction}`,
      );
      ...
      this.logger.error(
        `Error enhancing prompt: ${error.message || 'Unknown error'}`,
        error.stack,
      );
  ...
  private basicEnhancement(
    params: PromptEnhancementParams,
    originalUserInput: string,
  ): string {
    const { style } = params;
    const styleDescription =
      TattooStyleDescriptions[style as TattooStyle] || `style ${style}`;
    const subject = originalUserInput || 'tattoo design';

    return `
      Tattoo stencil design of ${subject},
      in the style of ${style}, characterized by ${styleDescription
      .split('|')
      .join(', ')},
