...
import { queues } from '../../../../queues/queues'; // Corrected path assuming this file is in src/queues/penalty/application/processors/
...
export class PenaltyProcessorService {
  ...
  constructor(
    ...
    @Inject(IReputationService) private readonly reputationService: IReputationService,
    @InjectQueue(queues.notification.name) private readonly notificationQueue: Queue,
    // @Inject(PaymentService) private readonly paymentService: PaymentService, // Placeholder
  ) {}

  @Process('PROCESS_PENALTY_V1')
  async handleProcessPenaltyV1(job: Job<ProcessPenaltyV1Job>): Promise<void> {
    const { penalty } = job.data;
    this.logger.log(`Processing penalty ID: ${penalty.id}, Type: ${penalty.type}, User ID: ${penalty.userId}`);

    try {
      // TODO: Point 2 & 3 - Implement actual penalty application logic here
      if ((penalty.type === PenaltyType.FIXED_FEE || penalty.type === PenaltyType.PERCENTAGE) && penalty.amount != null && penalty.amount > 0) {
        this.logger.log(`Placeholder: Applying financial penalty of ${penalty.amount} to user ${penalty.userId}. Type: ${penalty.type}`);
        // await this.paymentService.chargeUser(penalty.userId, penalty.amount, `Cancellation penalty for event ${penalty.eventId}`);
      }

      if (penalty.type === PenaltyType.REPUTATION_POINTS && penalty.reputationPoints != null && penalty.reputationPoints !== 0) {
        this.logger.log(`Applying ${penalty.reputationPoints} reputation points to user ${penalty.userId} for event ${penalty.eventId}.`);
        await this.reputationService.adjustReputation(penalty.userId, penalty.reputationPoints, `Cancellation penalty for event ${penalty.eventId}`);
      }

      // Mark penalty as applied if successfully processed by other services
      await this.cancellationPenaltyRepository.update(penalty.id, { status: PenaltyStatus.APPLIED });
      ...
      try {
        ...
        this.logger.log(`Dispatched PENALTY_APPLIED_NOTIFICATION_V1 job for penalty ID ${penalty.id} to notification queue.`);
      } catch (notifyError) {
        this.logger.error(`Failed to dispatch PENALTY_APPLIED_NOTIFICATION_V1 job for penalty ${penalty.id}. Error: ${(notifyError instanceof Error ? notifyError.message : String(notifyError))}`);
        // Non-critical, so don't re-throw, but log prominently.
      }

    } catch (error) {
      this.logger.error(`Failed to process penalty ID: ${penalty.id}. Error: ${(error instanceof Error ? error.message : String(error))}`, (error instanceof Error ? error.stack : undefined));
