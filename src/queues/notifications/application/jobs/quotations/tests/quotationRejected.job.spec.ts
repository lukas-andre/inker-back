...
import { QuotationRejectedJob } from '../quotationRejected.job';

describe('QuotationRejectedJob', () => {
  let job: QuotationRejectedJob;
  ...
  it('should send an email with the correct data when a quotation is rejected', async () => {
    const sendEmailSpy = jest.spyOn(emailService, 'sendEmail');

    const jobMetadata = { artistId: 1, customerId: 1, quotationId: 1 };
    const job = jobHandlerFactory.create({
      jobId: 'QUOTATION_REJECTED',
