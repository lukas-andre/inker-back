...
import { QuotationAcceptedJob } from '../quotationAccepeted.job';

describe('QuotationAcceptedJob', () => {
  let job: QuotationAcceptedJob;
  ...
  it('should send an email with the correct data when a quotation is accepted', async () => {
    const sendEmailSpy = jest.spyOn(emailService, 'sendEmail');

    const jobMetadata = { artistId: 1, customerId: 1, quotationId: 1 };
    const job = jobHandlerFactory.create({
      jobId: 'QUOTATION_ACCEPTED',
