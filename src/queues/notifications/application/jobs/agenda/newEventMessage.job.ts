...
import {
  ...
  NewEventMessageJob as NewEventMessageJobPayload,
...
export class NewEventMessageJob extends NotificationJob {
  ...
  async handle(job: NewEventMessageJobPayload): Promise<void> {
    ...
    this.logger.log(
      `Handling ${NewEventMessageJobId} for event ${eventId}, from sender ${senderId} (${senderUserType}) to receiver ID ${receiverUserTypeId}`,
    );
    ...
    try {
      ...
      if (senderUserType === UserType.ARTIST) {
        ...
        const customer = await this.customerProvider.findById(
          receiverUserTypeId,
        ); // Use findById with customer's own ID
        if (customer && customer.userId) {
          receiverGlobalUserId = customer.userId;
          receiverExists = true;
        } else {
          this.logger.error(
            `Receiver (Customer) not found or has no userId for customerId ${receiverUserTypeId}.`,
          );
        }
        ...
        if (artist && artist.userId) {
          ...
          this.logger.error(
            `Receiver (Artist) not found or has no userId for artistId ${receiverUserTypeId}.`,
          );
        }
      } else {
        this.logger.error(
          `Unknown senderUserType: ${senderUserType}. Cannot determine receiver type.`,
        );
        return;
      }
      ...
      if (!receiverExists || !receiverGlobalUserId) {
        this.logger.error(
          `Failed to identify receiverGlobalUserId for receiverUserTypeId ${receiverUserTypeId}. Aborting notification.`,
        );
        return;
      }
      ...
      this.logger.log(
        `Push notification sent to user ${receiverGlobalUserId} for event ${eventId}`,
      );
      ...
      this.logger.error(
        `Error handling ${NewEventMessageJobId} for event ${eventId}: ${e.message}`,
        e.stack,
      );
    }
  }
