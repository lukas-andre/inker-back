...
export class OfferAcceptedJob implements NotificationJob {
    ...
    async handle(job: OfferAcceptedJobType): Promise<void> {
        ...
        this.logger.log(`Handling OFFER_ACCEPTED job for offer ${offerId} to artist ${acceptedArtistId}`);
        ...
        try {
            ...
            this.logger.log(`Sent OFFER_ACCEPTED push notification to artist ${acceptedArtistId} (user: ${artist.userId})`);
            ...
            this.logger.error(
                `Error processing OFFER_ACCEPTED job for offer ${offerId}: ${(error as Error).message}`,
