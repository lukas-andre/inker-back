...
export class NewOfferReceivedJob implements NotificationJob {
    ...
    async handle(job: NewOfferReceivedJobType): Promise<void> {
        ...
        try {
            ...
            const artistName = artist ? `${artist.firstName} ${artist.lastName}` : 'An artist';
            ...
            await this.pushNotificationService.sendToUser(customer.userId, payload.notification, payload.data);
            this.logger.log(`Sent NEW_OFFER_RECEIVED push notification to customer ${customerId} (user: ${customer.userId})`);
            ...
            this.logger.error(
                `Error processing NEW_OFFER_RECEIVED job for offer ${offerId}: ${(error as Error).message}`,
