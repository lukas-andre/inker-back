...
export class OfferRejectedJob implements NotificationJob {
  ...
  async handle(job: OfferRejectedJobType): Promise<void> {
    ...
    this.logger.log(
      `Handling OFFER_REJECTED job for offer ${offerId} to artist ${rejectedArtistId}`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Sent OFFER_REJECTED push notification to artist ${rejectedArtistId} (user: ${artist.userId})`,
      );
      ...
      this.logger.error(
        `Error processing OFFER_REJECTED job for offer ${offerId}: ${
