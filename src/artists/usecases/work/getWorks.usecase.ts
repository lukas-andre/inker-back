...
export class GetWorksUseCase extends BaseUseCase {
  ...
  async execute(params: {
    ...
    if (onlyFeatured) {
      works = await this.workProvider.findFeaturedWorksByArtistId(
        artistId,
        includeHidden,
        source as WorkSource,
      );
    } else {
      works = await this.workProvider.findWorksByArtistId(
        artistId,
        includeHidden,
        source as WorkSource,
      );
    ...
    return includeMetrics
      ? await this.metricsEnricher.enrichAllWithMetrics(
          works,
          ContentType.WORK,
          userId,
          options,
