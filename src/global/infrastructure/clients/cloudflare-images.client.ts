...
export class CloudflareImagesClient {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.deliveryUrl = this.configService.get<string>('cloudflare.imagesDeliveryUrl');
      ...
      if (axios.isAxiosError(error) && error.response?.data) {
        ...
        if (cfError.errors?.length > 0) {
          ...
          switch (firstError.code) {
            case 5415:
              throw new Error('Unsupported image format. Supported formats: JPEG, PNG, WebP, GIF, SVG');
            case 5413:
              throw new Error('Image too large. Maximum size is 10MB');
            case 5455:
              throw new Error('Image format not supported');
            case 5559:
              throw new Error('Cloudflare API temporarily unavailable. Please retry');
            default:
  ...
  async delete(imageId: string): Promise<void> {
    try {
      const response = await this.axiosInstance.delete(`${this.baseUrl}/${imageId}`);
      
      if (!response.data.success) {
        throw new Error(`Failed to delete image: ${JSON.stringify(response.data.errors)}`);
      }
    } catch (error) {
      this.logger.error(`Failed to delete image ${imageId} from Cloudflare`, error);
      throw error;
    }
  }
  ...
  async listImages(page: number = 1, perPage: number = 100): Promise<{
    ...
    try {
      ...
      if (!response.data.success) {
        throw new Error(`Failed to list images: ${JSON.stringify(response.data.errors)}`);
      }
  ...
  async getImageDetails(imageId: string): Promise<CloudflareImageUploadResult> {
    try {
      const response = await this.axiosInstance.get(`${this.baseUrl}/${imageId}`);

      if (!response.data.success) {
        throw new Error(`Failed to get image details: ${JSON.stringify(response.data.errors)}`);
      }
  ...
  async updateImage(
    ...
    try {
      ...
      const response = await this.axiosInstance.patch(`${this.baseUrl}/${imageId}`, data);

      if (!response.data.success) {
        throw new Error(`Failed to update image: ${JSON.stringify(response.data.errors)}`);
      }
