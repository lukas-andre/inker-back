...
@Injectable()
export class ReviewRepository extends BaseComponent {
  constructor(
  ...
  async findByEventIds(eventsIds: string[]) {
    try {
  ...
  async findByArtistIds(artistId: string[]): Promise<FindByArtistIdsResult[]> {
  ...
  async isReviewRated(reviewId: string): Promise<boolean | undefined> {
    try {
  ...
  async findCustomerReviewsReactionDetail(
  ...
  ): Promise<Map<string, CustomerReviewReactionDetailsResult>> {
    const QueryResultSchema = z.object({
      reviewReactionId: z.string(),
      reviewId: z.string(),
      reactionType: z.enum([
    ...
    const resultMap = new Map<string, CustomerReviewReactionDetailsResult>();
