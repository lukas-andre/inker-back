...
@Injectable()
export class ListCustomerOpenQuotationsUseCase
  extends BaseUseCase
  implements UseCase
{
  constructor(
  ...
  async execute(
    ...
    const rawQuotations =
      await queryBuilder.getRawMany<OpenQuotationListItemDto>();
    ...
    if (quotationIds.length > 0) {
      const offers = await this.quotationOfferRepo.findByQuotationIdsNative(
        quotationIds,
      );
      ...
      if (offerArtistIds.length > 0) {
        ...
        artistNameMap = new Map(
          artists.map(a => [a.id, `${a.firstName} ${a.lastName}`]),
        );
      }
    ...
    if (customerIds.length > 0) {
      ...
      customerMap = new Map(
        customers.map(c => [c.id, c as unknown as CustomerDto]),
      );
    }

    // 4. Combine quotations with their offers and customer info
    const results: OpenQuotationListItemDto[] = rawQuotations.map(
      quotation => ({
        ...quotation,
