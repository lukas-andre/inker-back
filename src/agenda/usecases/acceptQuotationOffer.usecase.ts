...
import { DataSource, In, Not, Equal } from 'typeorm';
...
@Injectable()
export class AcceptQuotationOfferUseCase extends BaseUseCase implements UseCase, OnModuleDestroy {
    constructor(
    ...
    async execute(
        ...
        try {
            ...
            const quotation = await queryRunner.manager.findOne(this.quotationRepo.repo.target, {
            ...
            if (quotation.type !== QuotationType.OPEN) {
                throw new DomainForbidden('Cannot accept offer for a non-open quotation');
            }
            if (quotation.status !== QuotationStatus.OPEN) {
                // Allow accepting only from OPEN status? Or AWAITING_SELECTION?
                throw new DomainConflict(`Quotation is not in OPEN status (current: ${quotation.status})`);
            }
            ...
            if (otherOffers.length > 0) {
                ...
                await queryRunner.manager.update(QuotationOffer, { id: In(otherOfferIds) }, {
                    status: QuotationOfferStatus.REJECTED,
                });
            }

            // 6. Update Original Quotation
            await queryRunner.manager.update(this.quotationRepo.repo.target, quotationId, {
                ...
                status: QuotationStatus.QUOTED, // Transition to QUOTED state
            ...
            this.logger.error(`Failed to accept offer ${offerId} for quotation ${quotationId}: ${(error as Error).message}`, (error as Error).stack);
            // Re-throw specific errors or a generic one
            if (error instanceof DomainNotFound || error instanceof DomainForbidden || error instanceof DomainConflict || error instanceof DomainBadRule) {
                throw error;
            }
        ...
        try {
            ...
            for (const rejectedArtistId of rejectedOfferArtistIds) {
                 ...
                 const rejectedOffer = await this.offerRepo.findOne({ where: { quotationId, artistId: rejectedArtistId }});
                 if(rejectedOffer){
            ...
            this.logger.error(`Failed to add offer acceptance/rejection notification jobs for quotation ${quotationId}: ${(queueError as Error).message}`, (queueError as Error).stack);
             ...
             if (acceptedOfferArtistId) {
                const syncJob: CreateAgendaEventJobType = {
                    ...
                    metadata: {
                        ...
                        quotationId: quotationId,
                        // Add other necessary data if needed by the sync processor
                        // offerId: offerId,
                        // customerId: customerId,
                        // estimatedCost: acceptedOffer.estimatedCost, // Consider if needed
                        // estimatedDuration: acceptedOffer.estimatedDuration, // Consider if needed
                    },
                };
                await this.syncQueue.add(syncJob);
                this.logger.log(`Dispatched CREATE_AGENDA_EVENT job for quotation ${quotationId}`);
            }
        } catch (syncQueueError) {
             this.logger.error(`Failed to add CREATE_AGENDA_EVENT job for quotation ${quotationId}: ${(syncQueueError as Error).message}`, (syncQueueError as Error).stack);
