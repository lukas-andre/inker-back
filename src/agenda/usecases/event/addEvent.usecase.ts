  ...
  implements UseCase, OnModuleDestroy
{
  ...
  async execute(addEventDto: AddEventReqDto): Promise<void> {
    ...
    if (!eventEntity) {
      this.logger.error(
        `Failed to find newly created event ${result.eventId} for state transition.`,
      );
      throw new DomainNotFound(
        'Newly created event not found for state transition',
      );
    }
    ...
    const eventContext: StateMachineContext = {
      ...
      payload: {
        notes:
          'Event created directly by artist; confirmation automatically requested.',
      },
    };
    ...
    try {
      ...
      this.logger.log(
        `Event ${result.eventId} transitioned from CREATED to ${newStatus} via request_confirmation.`,
      );
    } catch (error) {
      this.logger.error(
        `Error transitioning event ${result.eventId} after creation:`,
        error,
      );
      const errorMessage =
      ...
      throw new DomainBadRule(
        `Error transitioning event to pending confirmation: ${errorMessage}`,
      );
    }
  }
}
