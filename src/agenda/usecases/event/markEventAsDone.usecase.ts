...
export class MarkEventAsDoneUseCase extends BaseUseCase implements UseCase {
  ...
  async execute(
    ...
    if (!agenda) {
        ...
        if (!agenda) {
            throw new DomainNotFound(`Agenda with ID ${agendaIdFromRequest} not found.`);
        }
        event.agenda = agenda; // Associate for consistency
    }
    
    if (agenda.id !== agendaIdFromRequest) {
        throw new DomainForbidden('Event agenda ID does not match requested agenda ID.');
    }

    if (agenda.artistId !== artistId) {
        throw new DomainForbidden('User is not authorized to mark this event as done.');
    }
    ...
    const terminalCompletedStates = [AgendaEventStatus.COMPLETED, AgendaEventStatus.WAITING_FOR_PHOTOS, AgendaEventStatus.REVIEWED, AgendaEventStatus.CANCELED];
    ...
    if (workEvidenceFiles && workEvidenceFiles.length > 0) {
      ...
      if (eventAfterCompletion.status !== AgendaEventStatus.COMPLETED && eventAfterCompletion.status !== AgendaEventStatus.WAITING_FOR_PHOTOS) {
        this.logger.warn(`Event ${eventId} is in status ${eventAfterCompletion.status}, which may not be ready for ADD_PHOTOS. Proceeding, but ensure state machine guards this.`);
      ...
      const processedWorkEvidence: any = await this.multimediasService.handleWorkEvidenceMultimedias(
          workEvidenceFiles,
          eventId,
          agenda.id,
      );
