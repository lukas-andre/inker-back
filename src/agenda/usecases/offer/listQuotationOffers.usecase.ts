...
export class ListQuotationOffersUseCase extends BaseUseCase implements UseCase {
  ...
  async execute(
    ...
    if (quotation.status !== QuotationStatus.OPEN) {
      this.logger.warn(
        `Listing offers for quotation ${quotationId} in status ${quotation.status}`,
      );
    }
    if (!quotation.customerLat || !quotation.customerLon) {
      this.logger.error(
        `Quotation ${quotationId} is OPEN but missing customer location.`,
      );
      throw new DomainNotFound(
        'Internal Server Error: Quotation location missing',
      );
    }
    ...
    const offerDtos: QuotationOfferListItemDto[] = offers.map(offer => {
      ...
      if (artistEntity) {
        ...
        if (artistLocation && artistLocation.lat && artistLocation.lng) {
          ...
          this.logger.warn(
            `Primary location not found for artist ${offer.artistId}`,
          );
        }
      } else {
        this.logger.warn(
          `Artist details not found for artist ${offer.artistId}`,
        );
      }
