...
export class QuotationEnrichmentService {
  ...
  async enrichQuotations(
    ...
    const customerIds = options.includeCustomer
      ? [...new Set(quotations.map(q => q.customerId).filter(Boolean))]
      : [];
    const artistIds = options.includeArtist
      ? [...new Set(quotations.map(q => q.artistId).filter(Boolean))]
      : [];
    const stencilIds = options.includeStencil
      ? [...new Set(quotations.map(q => q.stencilId).filter(Boolean))]
      : [];
    const tattooDesignCacheIds = options.includeTattooDesignCache
      ? [...new Set(quotations.map(q => q.tattooDesignCacheId).filter(Boolean))]
      : [];
    ...
    const [
      ...
      options.includeCustomer && customerIds.length > 0
        ? this.customerRepo.find({ where: { id: In(customerIds) } })
        : [],
      options.includeArtist && artistIds.length > 0
        ? this.artistRepo.find({ where: { id: In(artistIds) } })
        : [],
      options.includeStencil && stencilIds.length > 0
        ? Promise.all(
            stencilIds.map(id => this.stencilRepo.findStencilById(id)),
          )
        : [],
      options.includeTattooDesignCache && tattooDesignCacheIds.length > 0
        ? Promise.all(
            tattooDesignCacheIds.map(id =>
              this.tattooDesignCacheRepo.findById(id),
            ),
          )
        : [],
      options.includeLocation && artistIds.length > 0
        ? this.locationRepo.find({ where: { artistId: In(artistIds) } })
        : [],
      options.includeOffers
        ? this.offerRepo.findByQuotationIdsNative(quotations.map(q => q.id))
        : [],
    ]);

    // 3. Mapear entidades a mapas para acceso rápido
    const customerMap = new Map<string, CustomerDto>(
      customers.map((c: CustomerDto): [string, CustomerDto] => [c.id, c]),
    );
    const artistMap = new Map<string, ArtistDto>(
      artists.map((a: ArtistDto): [string, ArtistDto] => [a.id, a]),
    );
    const stencilMap = new Map<string, Stencil>(
      stencils
        .filter(Boolean)
        .map((s: Stencil): [string, Stencil] => [s.id, s]),
    );
    const tattooDesignCacheMap = new Map<string, TattooDesignCacheEntity>(
      tattooDesignCaches
        .filter(Boolean)
        .map(
          (tdc: TattooDesignCacheEntity): [string, TattooDesignCacheEntity] => [
            tdc.id,
            tdc,
          ],
        ),
    );
    const locationMap = new Map<string, ArtistLocation>(
      locations.map((l: ArtistLocation): [string, ArtistLocation] => [
        l.artistId,
        l,
      ]),
    );
    ...
    const offersMap = new Map<string, OpenQuotationOfferDto[]>();
    ...
    const artistOfferMap = new Map<string, boolean>();
    if (
      options.includeHasOffered &&
      options.currentArtistId &&
      offers.length > 0
    ) {
    ...
    return quotations.map(q => {
      const dto: GetQuotationResDto = {
        ...
        stencil:
          options.includeStencil && q.stencilId
            ? stencilMap.get(q.stencilId)
            : undefined,
        tattooDesignCache:
          options.includeTattooDesignCache && q.tattooDesignCacheId
            ? tattooDesignCacheMap.get(q.tattooDesignCacheId)
            : undefined,
        location:
          options.includeLocation && q.artistId
            ? locationMap.get(q.artistId)
            : undefined,
        offers: options.includeOffers ? offersMap.get(q.id) || [] : undefined,
        hasOffered:
          options.includeHasOffered && options.currentArtistId
            ? !!artistOfferMap.get(q.id)
            : undefined,
      };
      return dto;
    });
