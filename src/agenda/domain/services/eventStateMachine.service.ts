...
export class EventStateMachineService extends BaseComponent {
  ...
  // --- GUARDS ---
  private async canCustomerCancelGuard(
    context: StateMachineContext,
  ): Promise<boolean> {
    if (context.actor.role === UserType.CUSTOMER) {
      const hoursTillAppointment = this.calculateHoursDifference(
        new Date(),
        context.eventEntity.startDate,
      );
      if (hoursTillAppointment < 24) {
        this.logger.warn(
          `Customer ${context.actor.userId} cancel prevented for event ${context.eventEntity.id}: < 24 hours.`,
        );
        return false;
      }
    }
  ...
  private async canCustomerRescheduleGuard(
    context: StateMachineContext,
  ): Promise<boolean> {
    if (context.actor.role === UserType.CUSTOMER) {
      const hoursTillAppointment = this.calculateHoursDifference(
      ...
      if (hoursTillAppointment < 48) {
        this.logger.warn(
          `Customer ${context.actor.userId} reschedule prevented for event ${context.eventEntity.id}: < 48 hours.`,
        );
        return false;
      }
    }
  ...
  private async hasNotExceededRescheduleRequestLimitGuard(
    context: StateMachineContext,
  ): Promise<boolean> {
    ...
    const recentReschedulesByActor = event.rescheduleLog.filter(
      log =>
        ...
        log.actorId === context.actor.userId,
    ).length;

    if (recentReschedulesByActor >= 3) {
      this.logger.warn(
        `Reschedule limit (3 in 7 days) reached for event ${event.id} by actor ${context.actor.userId}`,
      );
      return false;
    }
    return true;
  ...
  private async hasRequiredConsentsSignedGuard(
    context: StateMachineContext,
  ): Promise<boolean> {
    ...
    if (context.actor.role !== UserType.CUSTOMER) {
      this.logger.log(
        `Consent check skipped for event ${context.eventEntity.id}: Actor is not a customer.`,
      );
      return true;
    }
    ...
    try {
      ...
      if (!signedConsent) {
        this.logger.warn(
          `Confirmation prevented for event ${context.eventEntity.id}: Customer ${context.actor.userId} has not signed required consent.`,
        );
        return false;
      }

      this.logger.log(
        `Consent check passed for event ${context.eventEntity.id}: Customer ${context.actor.userId} has signed consent.`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `Error checking consent for event ${context.eventEntity.id}:`,
        error,
      );
  ...
  private async dispatchPendingConfirmationNotification(
    context: StateMachineContext,
  ): Promise<void> {
    if (!context.eventEntity.customerId) {
      this.logger.warn(
        `Event ${context.eventEntity.id} has no customerId, skipping pending confirmation notification.`,
      );
      return;
    }
    ...
    const jobPayload: AgendaEventStatusChangedJobType = {
      ...
      metadata: {
        ...
        artistId:
          context.eventEntity.agenda?.artistId ||
          context.payload?.artistId ||
          'unknown_artist',
    ...
    try {
      ...
      this.logger.log(
        `Dispatched EVENT_STATUS_CHANGED job for event ${context.eventEntity.id} entering PENDING_CONFIRMATION`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to dispatch notification for event ${context.eventEntity.id} entering PENDING_CONFIRMATION`,
        error,
      );
  ...
  private async dispatchPaymentPendingNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId && !eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId or artistId, skipping payment pending notification.`,
      );
      return;
    }
    ...
    if (eventEntity.customerId) {
      await this.notificationQueue
        ...
        .catch(e =>
          this.logger.error(
            `Failed to dispatch PAYMENT_PENDING notification to customer ${eventEntity.customerId} for event ${eventEntity.id}`,
            e,
          ),
        );
      this.logger.log(
        `Dispatched PAYMENT_PENDING notification to customer for event ${eventEntity.id}`,
      );
    }
  ...
  private async dispatchSessionStartedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId && !eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId or artistId, skipping session started notification.`,
      );
      return;
    }
    ...
    if (eventEntity.customerId) {
      await this.notificationQueue
        ...
        .catch(e =>
          this.logger.error(
            `Failed to dispatch IN_PROGRESS notification to customer ${eventEntity.customerId} for event ${eventEntity.id}`,
            e,
          ),
        );
      this.logger.log(
        `Dispatched IN_PROGRESS notification to customer for event ${eventEntity.id}`,
      );
    }
    if (eventEntity.agenda?.artistId) {
      await this.notificationQueue
        ...
        .catch(e =>
          this.logger.error(
            `Failed to dispatch IN_PROGRESS notification to artist ${eventEntity.agenda?.artistId} for event ${eventEntity.id}`,
            e,
          ),
        );
      this.logger.log(
        `Dispatched IN_PROGRESS notification to artist for event ${eventEntity.id}`,
      );
    }
  }

  private async dispatchSessionCompletedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId && !eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId or artistId, skipping session completed notification.`,
      );
      return;
    }
    ...
    if (eventEntity.customerId) {
      await this.notificationQueue
        ...
        .catch(e =>
          this.logger.error(
            `Failed to dispatch COMPLETED notification to customer ${eventEntity.customerId} for event ${eventEntity.id}`,
            e,
          ),
        );
      this.logger.log(
        `Dispatched COMPLETED notification to customer for event ${eventEntity.id}`,
      );
    }
    if (eventEntity.agenda?.artistId) {
      await this.notificationQueue
        ...
        .catch(e =>
          this.logger.error(
            `Failed to dispatch COMPLETED notification to artist ${eventEntity.agenda?.artistId} for event ${eventEntity.id}`,
            e,
          ),
        );
      this.logger.log(
        `Dispatched COMPLETED notification to artist for event ${eventEntity.id}`,
      );
    }
  }

  private async dispatchRequestPhotosNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    ...
    if (!eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no artistId, skipping request photos notification.`,
      );
      return;
    }
    ...
    try {
      await this.notificationQueue.add(jobPayload);
      this.logger.log(
        `Dispatched WAITING_FOR_PHOTOS notification to artist for event ${eventEntity.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to dispatch WAITING_FOR_PHOTOS notification to artist for event ${eventEntity.id}`,
        error,
      );
    }
  }

  private async dispatchRequestReviewNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId, skipping request review notification.`,
      );
      return;
    }
    ...
    const jobPayload: AgendaEventStatusChangedJobType = {
      ...
      metadata: {
        ...
        customerId: eventEntity.customerId,
        artistId:
          eventEntity.agenda?.artistId || payload?.artistId || 'unknown_artist',
    ...
    try {
      await this.notificationQueue.add(jobPayload);
      this.logger.log(
        `Dispatched WAITING_FOR_REVIEW notification to customer for event ${eventEntity.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to dispatch WAITING_FOR_REVIEW notification to customer for event ${eventEntity.id}`,
        error,
      );
    }
  }

  private async dispatchReviewAddedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    ...
    if (!eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no artistId, skipping review added notification.`,
      );
      return;
    }
    ...
    try {
      await this.notificationQueue.add(jobPayload);
      this.logger.log(
        `Dispatched REVIEWED notification to artist for event ${eventEntity.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to dispatch REVIEWED notification to artist for event ${eventEntity.id}`,
        error,
      );
    }
    ...
    if (eventEntity.customerId) {
      ...
      try {
        ...
        this.logger.log(
          `Dispatched REVIEWED confirmation to customer for event ${eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch REVIEWED confirmation to customer for event ${eventEntity.id}`,
          error,
        );
      }
    }
  ...
  private async dispatchAftercareStartedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId, skipping aftercare started notification.`,
      );
      return;
    }
    ...
    try {
      await this.notificationQueue.add(jobPayload);
      this.logger.log(
        `Dispatched AFTERCARE_PERIOD notification to customer for event ${eventEntity.id}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to dispatch AFTERCARE_PERIOD notification to customer for event ${eventEntity.id}`,
        error,
      );
    }
  }

  private async dispatchDisputeOpenedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    if (!eventEntity.customerId && !eventEntity.agenda?.artistId) {
      this.logger.warn(
        `Event ${eventEntity.id} has no customerId or artistId, skipping dispute opened notification.`,
      );
      return;
    }
    ...
    if (eventEntity.customerId) {
      ...
      try {
        await this.notificationQueue.add(customerJobPayload);
        this.logger.log(
          `Dispatched DISPUTE_OPEN notification to customer for event ${eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch DISPUTE_OPEN notification to customer for event ${eventEntity.id}`,
          error,
        );
      }
    }
    ...
    if (eventEntity.agenda?.artistId) {
      ...
      try {
        ...
        this.logger.log(
          `Dispatched DISPUTE_OPEN notification to artist for event ${eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch DISPUTE_OPEN notification to artist for event ${eventEntity.id}`,
          error,
        );
      }
    }
  ...
  private async dispatchConfirmedNotification(
    context: StateMachineContext,
  ): Promise<void> {
    if (
      !context.eventEntity.customerId &&
      ...
      this.logger.warn(
        `Event ${context.eventEntity.id} has no customerId or artistId, skipping confirmed notification.`,
      );
      return;
    }

    const message = `Appointment '${
      context.eventEntity.title
    }' has been confirmed and is scheduled for ${new Date(
      context.eventEntity.startDate,
    ).toLocaleString()}.`;
    const baseJobMetadata = {
      eventId: context.eventEntity.id,
    ...
    if (context.eventEntity.customerId) {
      ...
      try {
        await this.notificationQueue.add(customerJobPayload);
        this.logger.log(
          `Dispatched CONFIRMED notification to customer for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch CONFIRMED notification to customer for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
    ...
    if (context.eventEntity.agenda?.artistId) {
      ...
      try {
        await this.notificationQueue.add(artistJobPayload);
        this.logger.log(
          `Dispatched CONFIRMED notification to artist for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch CONFIRMED notification to artist for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
  ...
  private async dispatchCanceledNotification(
    context: StateMachineContext,
    eventTrigger?: AgendaEventTransition,
  ): Promise<void> {
    let message = '';
    if (
      ...
      this.logger.warn(
        `Event ${context.eventEntity.id} has no customerId or artistId, skipping canceled notification.`,
      );
      return;
    }
    ...
    if (context.eventEntity.customerId) {
      ...
      try {
        await this.notificationQueue.add(customerJobPayload);
        this.logger.log(
          `Dispatched CANCELED notification to customer for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch CANCELED notification to customer for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
    ...
    if (context.eventEntity.agenda?.artistId) {
      ...
      try {
        await this.notificationQueue.add(artistJobPayload);
        this.logger.log(
          `Dispatched CANCELED notification to artist for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch CANCELED notification to artist for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
  ...
  private async dispatchRescheduledNotification(
    context: StateMachineContext,
  ): Promise<void> {
    if (
      !context.eventEntity.customerId &&
      ...
      this.logger.warn(
        `Event ${context.eventEntity.id} has no customerId or artistId, skipping rescheduled notification.`,
      );
      return;
    }
    ...
    const message = `Appointment '${
      ...
      context.eventEntity.startDate,
    ).toLocaleString()}. Please review and confirm if necessary.`;
    const baseJobMetadata = {
      eventId: context.eventEntity.id,
    ...
    if (context.eventEntity.customerId) {
      ...
      try {
        await this.notificationQueue.add(customerJobPayload);
        this.logger.log(
          `Dispatched RESCHEDULED notification to customer for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch RESCHEDULED notification to customer for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
    ...
    if (context.eventEntity.agenda?.artistId) {
      ...
      try {
        await this.notificationQueue.add(artistJobPayload);
        this.logger.log(
          `Dispatched RESCHEDULED notification to artist for event ${context.eventEntity.id}`,
        );
      } catch (error) {
        this.logger.error(
          `Failed to dispatch RESCHEDULED notification to artist for event ${context.eventEntity.id}`,
          error,
        );
      }
    }
  ...
  private async updateEventDatesForReschedule(
    context: StateMachineContext,
  ): Promise<void> {
    const { eventEntity, payload } = context;
    ...
    const originalEndDateForLog = eventEntity.endDate
      ? new Date(eventEntity.endDate.getTime())
      : undefined;

    if (payload?.newStartDate) {
      eventEntity.startDate = new Date(payload.newStartDate as string | Date); // Ensure it's a Date object
      this.logger.log(
        `Event ${eventEntity.id} startDate updated to ${eventEntity.startDate} during RESCHEDULE transition.`,
      );
    }
    if (payload?.newEndDate) {
      eventEntity.endDate = new Date(payload.newEndDate as string | Date); // Ensure it's a Date object
      this.logger.log(
        `Event ${eventEntity.id} endDate updated to ${eventEntity.endDate} during RESCHEDULE transition.`,
      );
    }
  ...
  private initializeStateMachine(): void {
    ...
    this.stateConfig = {
      ...
      [AgendaEventStatus.PENDING_CONFIRMATION]: {
        onEntry: [this.dispatchPendingConfirmationNotification.bind(this)],
        transitions: {
          [AgendaEventTransition.CONFIRM]: {
            target: AgendaEventStatus.CONFIRMED,
            guards: [this.hasRequiredConsentsSignedGuard.bind(this)],
          ...
          [AgendaEventTransition.REJECT]: {
            // Reject should lead to CANCELED
            target: AgendaEventStatus.CANCELED,
            // actions: [async (event, context) => console.log('Action: Notify rejection and cancellation')]
          },
          [AgendaEventTransition.CANCEL]: {
            // Allow cancellation while pending confirmation
            target: AgendaEventStatus.CANCELED,
          },
        },
      },
      [AgendaEventStatus.CONFIRMED]: {
        onEntry: [this.dispatchConfirmedNotification.bind(this)],
        transitions: {
          ...
          [AgendaEventTransition.INITIAL_SCHEDULE]: {
            // Direct to CONFIRMED if payment is not a separate step or already handled
      ...
      [AgendaEventStatus.PAYMENT_PENDING]: {
        onEntry: [this.dispatchPaymentPendingNotification.bind(this)],
        transitions: {
          [AgendaEventTransition.INITIAL_SCHEDULE]: {
            // Assuming payment completion triggers scheduling
            target: AgendaEventStatus.CONFIRMED, // Was SCHEDULED
          ...
          [AgendaEventTransition.CANCEL]: {
            ...
            actions: [
              (context, eventTrigger) =>
                this.dispatchCanceledNotification(context, eventTrigger),
            ],
          },
        },
      },
      [AgendaEventStatus.IN_PROGRESS]: {
        onEntry: [this.dispatchSessionStartedNotification.bind(this)],
        transitions: {
          ...
          [AgendaEventTransition.CANCEL]: {
            // Might be too late or have specific rules
            target: AgendaEventStatus.CANCELED,
      ...
      [AgendaEventStatus.COMPLETED]: {
        onEntry: [this.dispatchSessionCompletedNotification.bind(this)],
        transitions: {
      ...
      [AgendaEventStatus.REVIEWED]: {
        ...
        onEntry: [this.dispatchReviewAddedNotification.bind(this)],
        transitions: {
      ...
      [AgendaEventStatus.RESCHEDULED]: {
        // After rescheduling, it likely goes back to pending confirmation or confirmed
        onEntry: [this.dispatchRescheduledNotification.bind(this)],
        transitions: {
          [AgendaEventTransition.REQUEST_CONFIRMATION]: {
            // If re-confirmation is always needed by the other party
            target: AgendaEventStatus.PENDING_CONFIRMATION,
          ...
          [AgendaEventTransition.CONFIRM]: {
            // If the actor who rescheduled can also auto-confirm it (e.g. artist has full control or customer accepts new terms implicitly)
            target: AgendaEventStatus.CONFIRMED,
          ...
          [AgendaEventTransition.RESCHEDULE]: {
            ...
            actions: [this.updateEventDatesForReschedule.bind(this)],
          },
        },
      },
      ...
      [AgendaEventStatus.AFTERCARE_PERIOD]: {
        onEntry: [this.dispatchAftercareStartedNotification.bind(this)],
        transitions: {}, // Ensure transitions property exists
      },
      [AgendaEventStatus.DISPUTE_OPEN]: {
        onEntry: [this.dispatchDisputeOpenedNotification.bind(this)],
  ...
  async transition(
    ...
    if (!stateDefinition) {
      this.logger.error(
        `Invalid current state: ${currentState} for event ${context.eventEntity?.id}`,
      );
    ...
    if (!transition) {
      this.logger.error(
        `Event ${eventKey} not allowed for state ${currentState} for event ${context.eventEntity?.id}`,
      );
      throw new DomainUnProcessableEntity(
    ...
    if (transition.guards) {
      for (const guard of transition.guards) {
        if (!(await guard(context))) {
          this.logger.warn(
            `Guard prevented transition for event ${context.eventEntity?.id} from ${currentState} via ${eventKey}`,
          );
          throw new DomainUnProcessableEntity(
    ...
    try {
      ...
      this.logger.log(
        `Event ${context.eventEntity.id} transitioned from ${currentState} to ${targetState} via ${eventKey}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to save event ${context.eventEntity.id} after transition to ${targetState}`,
        error,
      );
  ...
  ): Readonly<(typeof this.stateConfig)[AgendaEventStatus]> {
    return this.stateConfig[state];
  }

  private async checkIfEventHasPhotos(
    eventEntity: AgendaEvent,
  ): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(
        `Error checking photos for event ${eventEntity.id}`,
        error,
      );
      return false;
    }
