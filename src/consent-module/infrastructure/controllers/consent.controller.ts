...
export class ConsentController {
  ...
  @ApiResponse({
    status: 403,
    description:
      'Forbidden - User does not have required role or artist ID mismatch',
  })
  async createTemplate(
    ...
    if (!userTypeId) {
      throw new HttpException(
        'Artist ID not found in token for an artist user.',
    ...
    if (userType !== UserType.ARTIST) {
      throw new HttpException(
        'User does not have required role to create a template.',
        HttpStatus.FORBIDDEN,
      );
    }
  ...
  @ApiParam({
    name: 'templateId',
    type: 'string',
    description: 'UUID of the form template',
  })
  @ApiResponse({
    status: 200,
  ...
  async getTemplate(
    ...
    const artistAccessingId =
      req.user.userType === UserType.ARTIST ? req.user.artistId : undefined;
  ...
  @ApiOperation({
    summary:
      'Get all active consent form templates for a specific artist (callable by the artist themselves)',
  })
  @ApiParam({
    name: 'artistId',
    type: 'string',
  ...
  @ApiResponse({
    status: 403,
    description:
      'Forbidden - User is not an artist or not accessing their own templates',
  })
  async getArtistTemplates(
    ...
    if (userType !== UserType.ARTIST) {
      throw new HttpException(
        'User does not have required role to access templates.',
        HttpStatus.FORBIDDEN,
      );
    }
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid input or consent conditions not met',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  async signConsent(
    ...
    return this.signConsentUseCase.execute(
      signConsentDto,
      userId,
      ipAddress,
      userAgent,
  ...
  @ApiResponse({
    status: 403,
    description:
      'Forbidden - User does not have required role or is not the template owner',
  })
  ...
  async updateTemplate(
    ...
    if (userType !== UserType.ARTIST) {
      throw new HttpException(
        'User does not have required role to update a template.',
        HttpStatus.FORBIDDEN,
      );
    }
  ...
  @ApiResponse({
    status: 400,
    description: 'Cannot delete template that has been used in signed consents',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({
    status: 403,
  ...
  async deleteTemplate(
    ...
    if (userType !== UserType.ARTIST) {
      throw new HttpException(
        'User does not have required role to delete a template.',
        HttpStatus.FORBIDDEN,
      );
    }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Update a consent form template status (activate/deactivate)',
  })
  @ApiParam({
    name: 'templateId',
    type: 'string',
  ...
  async updateTemplateStatus(
    ...
    if (userType !== UserType.ARTIST) {
      throw new HttpException(
        'User does not have required role to update template status.',
        HttpStatus.FORBIDDEN,
      );
    }

    return this.updateTemplateStatusUseCase.execute(
      templateId,
      updateTemplateStatusDto.isActive,
      userTypeId,
    );
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Accept default terms and conditions for an event (MVP endpoint)',
  })
  @ApiResponse({
    status: 201,
    description: 'Default terms accepted successfully',
    type: SignedConsentDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Invalid input or event status not suitable for acceptance',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({
    status: 403,
    description: 'Forbidden - Only customers can accept terms',
  })
  @ApiResponse({ status: 404, description: 'Event not found' })
  @ApiResponse({
    status: 409,
    description: 'Terms have already been accepted for this event',
  })
  async acceptDefaultTerms(
    ...
    if (!userTypeId) {
      throw new HttpException(
        'User ID not found in token for an artist user.',
        HttpStatus.FORBIDDEN,
      );
    }
    ...
    if (userType !== UserType.CUSTOMER) {
      throw new HttpException(
        'User does not have required role to accept terms.',
        HttpStatus.FORBIDDEN,
      );
    }
    return this.acceptDefaultTermsUseCase.execute(
      acceptDefaultTermsDto,
      userTypeId,
      ipAddress,
      userAgent,
    );
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Check if customer has signed consent for an event',
  })
  @ApiParam({
    name: 'eventId',
    type: 'string',
  ...
  @ApiResponse({
    status: 200,
    description: 'Consent status information',
    type: CheckConsentStatusDto,
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Event not found' })
  async checkConsentStatus(
    ...
    if (userType !== UserType.CUSTOMER) {
      throw new HttpException(
        'User does not have required role to check consent status.',
        HttpStatus.FORBIDDEN,
      );
    }
