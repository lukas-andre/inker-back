  ...
  async execute(
    ...
    if (createDto.artistId !== callingArtistId) {
      throw new ForbiddenException(
        'You are not authorized to create a template for this artist.',
      );
    }
    ...
    const existingTemplateWithSameDetails =
      await this.formTemplateRepository.findByArtistAndDetails(
        createDto.artistId,
        createDto.title,
