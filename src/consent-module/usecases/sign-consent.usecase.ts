...
export class SignConsentUseCase {
  constructor(
    ...
    private readonly agendaEventRepository: AgendaEventRepository, // Injecting concrete repository // @Inject(PdfGenerationService) // Optional: For direct PDF generation or queuing // private readonly pdfGenerationService: PdfGenerationService, // @InjectQueue(CONSENT_PDF_QUEUE) private readonly consentPdfQueue: Queue, // Optional: For Bull queue
  ) {}
  ...
  async execute(
    ...
    const event: AgendaEvent | null = await this.agendaEventRepository.findOne({
    ...
    if (!allowedEventStatuses.includes(event.status)) {
      throw new BadRequestException(
        `Consent cannot be signed for an event with status '${event.status}'.`,
      );
    }

    // Check if this user has already signed this specific consent for this event
    const existingConsent =
      await this.signedConsentRepository.findByEventUserAndTemplate(
        dto.eventId,
        userId,
        dto.formTemplateId,
      );
    if (existingConsent) {
      throw new ConflictException(
        'Consent already signed for this event' +
          (dto.formTemplateId ? ' and template.' : '.'),
      );
    }

    // Validate formTemplateId if provided
    if (dto.formTemplateId) {
      const formTemplate = await this.formTemplateRepository.findById(
        dto.formTemplateId,
      );
      if (!formTemplate) {
      ...
      if (!formTemplate.isActive) {
        throw new BadRequestException(
          `Form template with ID ${dto.formTemplateId} is not active.`,
        );
      }
      ...
      if (!event.agenda || !event.agenda.artistId) {
        ...
        throw new BadRequestException(
          'Event is not properly associated with an artist.',
        );
      }
      if (formTemplate.artistId !== event.agenda.artistId) {
        throw new ForbiddenException(
          'Form template does not belong to the event artist.',
        );
      }
    }
