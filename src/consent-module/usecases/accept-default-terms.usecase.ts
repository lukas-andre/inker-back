...
export class AcceptDefaultTermsUseCase {
    ...
    async execute(
        ...
        const existingConsent = await this.signedConsentRepository.findByEventAndUser(dto.eventId, userId);
        if (existingConsent) {
            throw new ConflictException('Terms and conditions have already been accepted for this event.');
        }
