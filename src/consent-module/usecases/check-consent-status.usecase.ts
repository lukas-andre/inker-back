...
export class CheckConsentStatusUseCase {
    ...
    async execute(eventId: string, userId: string): Promise<CheckConsentStatusDto> {
        ...
        const signedConsent = await this.signedConsentRepository.findByEventAndUser(eventId, userId);
