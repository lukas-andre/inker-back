...
export class PushNotificationService {
    ...
    constructor(
        private tokenRepository: NotificationRepository
    ) { }
    ...
    async saveToken(userId: number, token: string, deviceType: DeviceType) {
        try {
            ...
            if (error instanceof Error) {
                ...
                this.logger.error(`Error saving token: Unknown ${JSON.stringify(error)}`);
            }
            throw error;
        }
    }
        ...
        try {
            ...
            const message: admin.messaging.MulticastMessage = {
                ...
                apns: {
                    ...
                    headers: {
                        'apns-priority': '10',
                        'apns-expiration': Math.floor(Date.now() / 1000 + 24 * 3600).toString() // 24 hours
                    }
                },
                // webpush: {
                //     notification: {
            ...
            if (error instanceof Error) {
                ...
                this.logger.error(`Error sending notification: Unknown ${JSON.stringify(error)}`);
            }
            throw error;
        }
    }
